...
/// Fleet-wide KV-cache duplication in the router's live radix index: how
/// much of what's tracked is genuinely unique content versus copies of the
/// same block sitting on more than one worker (e.g. a session's prefix
/// re-materializing on a second worker after a router hop, or cache-aware
/// routing simply not being perfect). Diagnostic-only - see
